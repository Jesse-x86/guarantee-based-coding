...
1. **架构相(先草案,谈定才落库)。** 把本次变更涉及的**全部** gbc.md 增量作为**草案**(文本)交给人类
   讨论修改;只有**谈定之后**才经 gbc-doc skill 落库,并跑 `check` 确认干净。skill 只落已批准的内容。
   **已落库的 gbc.md = 你实现的 spec。**(意图由人类拍板是 GBC 的设计;正常情形下这是推荐流程。)
2. **实现相(机器门控)。** 按依赖**拓扑序**实现(先做被依赖者)。对每个文件:
   - 读已批准的 gbc.md + 你依赖的接口 → 写实现。
   - 想清楚"我依赖它的哪些**行为**":
     - 只依赖签名 / 符号存在 → `add_dependency(provider, consumer, symbol)`(免费)。
     - 依赖一个具体的**行为**(非空、异常语义……)→ 若命中一条已有保证,`add_dependency(..., guarantee_id=...)`
       复用它;若没有,**先写一个_窄_测试**,再 `create_guarantee(...)`。
   - 跑 `verify_provider` / `verify_guarantee`;这一步只有全绿才算过。
   - 如果实现到一半发现**意图本身**要改 → 回到架构相,重新起草并谈定,别偷偷漂移。
...
| 想清楚架构后就直接跳去写代码 | 先起草 gbc.md → 人类审批 → 再按拓扑序实现 |
