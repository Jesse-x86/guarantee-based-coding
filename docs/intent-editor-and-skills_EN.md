...
**中文版: [intent-editor-and-skills.md](./intent-editor-and-skills.md)**

> Entry points: [for-agents.md](./for-agents.md) (for agents) / [for-humans_EN.md](./for-humans_EN.md) (for humans). This page expands the "wrap the intent CLI as a skill" step from the [manual_EN.md](./manual_EN.md) (web editor, full command surface, SKILL.md example).

GBC has two tracks with different permissions:

- **Guarantees** — agents self-serve CRUD through MCP, machine-gated. See [integrate-mcp_EN.md](./integrate-mcp_EN.md).
...
**The three sections are visibility scopes — when "intent vs constraints" is unclear, ask per sentence "who needs to know this?"** `# 意图` = what an **outsider** needs to know (the public contract); `# 内部约束` = what **all internal files must coordinate on and outsiders don't** (package-private shared invariants); a `## file` entry = what **only that file itself** needs to know (private). This visibility axis doesn't conflict with the "identity → intent / rule → constraints" test above — it fills a gap in it, working mainly as a **sniffer for internal detail leaking up into intent**: the implementation base (Pydantic vs dataclass), the disk layout (metadata lands in `session.json`), an internal mechanism (how the type gate is wired) are all internal detail — they belong in **constraints / file entries**, not in `# 意图`. (It only catches internal detail floating up into intent, one direction; a rule an outsider must obey still lives in constraints — that test is unchanged.)

**gbc.md states the *current* state + prohibitions, not a *diff against the past*.** It is the *current* contract; these three are the most common drift-bait — write them in and they rot and mislead the agent working against them:

- **Stage / version words** (`MVP`, `v0`, `v1`, `for now`): the project stage is a **global fact** that belongs in one place (the project instruction file, e.g. `CLAUDE.md`); copied into every folder's gbc.md it becomes N copies that desync the moment the stage advances. Drop the stage hat; keep only the stage-free, checkable fact.
- **Roadmap / wishes** (`will grow into a complex X someday`, `the interface leaves room to swap the backend`): putting a currently-unactionable wish in the top contract only makes the agent think "the present version is a stopgap." The fix is **delete, or move it to an ADR / roadmap** — rewriting `will do X someday` into `aims to do X` is lipstick (a roadmap item doesn't belong in intent regardless of tense).
- **Migration narration** (`no longer uses X`, `changed to Y`, `used to…`): the *no-longer / changed-to* act carries no information — for someone reading the *current* contract, only "what it is now" or "what it must never do" is useful. It's either a **ghost** (X is long gone from the code, nobody would assume it) → delete and state the present positively; or it's actually a **regression guard** (that "no longer" means "**never** reintroduce X") → rewrite it as a positive teethed prohibition "never X" in `# 内部约束`.
- **The only legitimate reference to the past = an explicitly labeled historical note**: e.g. "legacy default is still X — that's a **historical fact**, distinct from the new default." The label (`historical fact` / `legacy-compat` / `deprecated`) exists precisely so it isn't misread as the current contract. A bare time-diff is the drift.
...
python3 server.py                          # 127.0.0.1:8765, path box left blank
python3 server.py --root /path/to/.gbc     # prefilled and auto-loaded
# also --port / --host
```
