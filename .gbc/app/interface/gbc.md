...
- **base 是唯一 IO/编排点**:路径解析(项目相对 POSIX)、meta 文件加载/保存、跨文件双向写、
  全局扫描都在这里;`core` 之下不碰磁盘。
- **依赖登记是跨两个文件的双向写**,由 base 兜底(consumer 的 `depends_on` ⇄ provider 的
  `dependents`),调用方不必手工在两处读写。
