...
- **阶段 / 版本词**(`MVP`、`v0`、`v1`、`目前先`):项目阶段是**全局事实**,只该住项目指令文件(如 `CLAUDE.md`);抄进每个文件夹的 gbc.md = N 份副本,阶段一推进必漏同步。删掉阶段帽子,只留无阶段词、可检的事实。
- **roadmap / 愿望**(`日后会长成复杂的 X`、`接口留好换后端的余地`):最高契约里放一个当前无法执行的愿望,只会让 agent 以为"现在这版是权宜之计"。正解 = **删,或移交 ADR / roadmap**——把「日后会 X」原地改成「目标是 X」是**换汤不换药**(roadmap 无论时态都不属意图)。
- **迁移旁白**(`不再用 X`、`改为 Y`、`曾经…`):`不再 / 改为` 这个动作本身不携带信息——对读**当前**契约的人,有用的只有"现在是什么"或"绝不做什么"。它要么是**鬼影**(X 已从代码死透、没人会假设它)→ 删,正面陈述现状;要么其实是**防回归禁令**(那个"不再"= "**禁止**重新引入 X")→ 写成正向有牙禁令「绝不 X」放进 `# 内部约束`。
- **唯一合法的"过去"引用 = 显式标注的历史注记**:如「旧档兼容默认仍是 X——那是**历史事实**,与新默认是两回事」。标注(`历史事实` / `旧档兼容` / `已弃用`)存在的意义,就是让它不被误读成当前契约。裸留的时间 diff 才是漂移。
